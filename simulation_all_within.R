...
for(iteration in 1:2000){
  ...
  ##################
  start.time         <- Sys.time()
  fs.H0              <- suppressMessages( lmer( yH0 ~ Cond1 * Cond2 + (Cond1 * Cond2 | ID) , data = data.sim ) )
  et.fs.H0           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  fs.H1              <- suppressMessages( lmer( yH1 ~ Cond1 * Cond2 + (Cond1 * Cond2 | ID) , data = data.sim ) )
  et.fs.H1           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  ps.H0              <- suppressMessages( lmer( yH0 ~ Cond1 * Cond2 + (Cond1 | ID) , data = data.sim ) )
  et.ps.H0           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  ps.H1              <- suppressMessages( lmer( yH1 ~ Cond1 * Cond2 + (Cond1 | ID) , data = data.sim ) )
  et.ps.H1           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  fi.H0              <- suppressMessages( lmer( yH0 ~ Cond1 * Cond2 + ( 1 | ID:Cond1:Cond2) , data = data.sim ) )
  et.fi.H0           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  fi.H1              <- suppressMessages( lmer( yH1 ~ Cond1 * Cond2 + ( 1 | ID:Cond1:Cond2) , data = data.sim ) )
  et.fi.H1           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  pi.H0              <- suppressMessages( lmer( yH0 ~ Cond1 * Cond2 + ( 1 | ID:Cond1) , data = data.sim ) )
  ...
  start.time         <- Sys.time()
  fm.H0              <- suppressMessages( lmer( yH0 ~ Cond1 * Cond2 + (Cond1 | ID:Cond2) , data = data.sim ) )
  et.fm.H0           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  fm.H1              <- suppressMessages( lmer( yH1 ~ Cond1 * Cond2 + (Cond1 | ID:Cond2) , data = data.sim ) )
  et.fm.H1           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  mn.H0              <- suppressMessages( lmer( yH0 ~ Cond1 * Cond2 + (1 | ID) , data = data.sim ) )
  et.mn.H0           <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  mn.H1              <- suppressMessages( lmer( yH1 ~ Cond1 * Cond2 + (1 | ID) , data = data.sim ) )
  ...
  write.csv2(do.call("rbind", output), file = "output-all-within.csv")
}
