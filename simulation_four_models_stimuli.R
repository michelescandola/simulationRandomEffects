...
## NTrials = number of trials each stimulus
...
for(iteration in 1:2000){
  ...
  ##################
  start.time         <- Sys.time()
  slope.H0           <- suppressMessages( lmer( yH0 ~ Group * WCond + (WCond | ID) + (1 | Stimulus) , data = data.sim ) )
  elapsed.time.sH0   <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  slope.H1 <- suppressMessages( lmer( yH1 ~ Group * WCond + (WCond | ID) + (1 | Stimulus) , data = data.sim ) )
  elapsed.time.sH1   <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  intC.H0 <- suppressMessages( lmer( yH0 ~ Group * WCond + ( 1 | ID/WCond) + (1 | Stimulus) , data = data.sim ) )
  elapsed.time.iCH0  <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  intC.H1 <- suppressMessages( lmer( yH1 ~ Group * WCond + ( 1 | ID/WCond) + (1 | Stimulus) , data = data.sim ) )
  elapsed.time.iCH1  <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  intP.H0 <- suppressMessages( lmer( yH0 ~ Group * WCond + ( 1 | ID:WCond) + (1 | Stimulus) , data = data.sim ) )
  elapsed.time.iPH0  <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  intP.H1 <- suppressMessages( lmer( yH1 ~ Group * WCond + ( 1 | ID:WCond) + (1 | Stimulus) , data = data.sim ) )
  elapsed.time.iPH1  <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  int0.H0 <- suppressMessages( lmer( yH0 ~ Group * WCond + ( 1 | ID) + (1 | Stimulus) , data = data.sim ) )
  elapsed.time.i0H0  <- difftime( Sys.time(), start.time , units = "secs" )
  
  start.time         <- Sys.time()
  int0.H1 <- suppressMessages( lmer( yH1 ~ Group * WCond + ( 1 | ID) + (1 | Stimulus) , data = data.sim ) )
  ...
  write.csv2(do.call("rbind", output), file = "output-simulation-random-effects-stimuli.csv")
}
